...
export default function Profile({ token }) {
  ...
  const update = async () => {
    try {
      await axios.patch("http://localhost:5000/api/admin/update", {
        currentPassword, newUsername, newPassword
      }, { headers: { Authorization: `Bearer ${token}` } });
      setMessage("✅ Admin info updated");
    } catch (err) {
      setMessage("❌ " + (err.response?.data?.error || "Error updating"));
    }
  };
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 bg-white shadow p-6 rounded">
      ...
      <button className="w-full bg-blue-600 text-white py-2 rounded" onClick={update}>Update</button>
